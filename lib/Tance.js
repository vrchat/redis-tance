...
class Tance{
    ...
    _argsToShaKey(args, argsToIgnore){
        /*
            convert a functions arguments into a stable search key
            {b: 2, a: 1} => a_1_b_2
        ...
        let searchIdHash = crypto.createHash("sha1").update(searchId).digest("hex");
    ...
    cache(fn, durationInSeconds, fnName, serializationFn, deserializationFn, ignoreArgs){
        ...
        return async (...args) => {
            ...
            let searchIdSha1 = this._argsToShaKey(args, ignoreArgs);

            let results = await this.mget(`${fnName}_${searchIdSha1}`, `${fnName}_${searchIdHash}`);
