...
class Tance{
    ...
    cache(fn, durationInSeconds, name, serializationFn, deserializationFn){
        ...
        return async (...args) => {
            ...
            if(result == null){
                ...
                await this.set(`${name}_${searchIdHash}`, serializedReturnVal, 'EX', durationInSeconds);
                await this.sadd(`${name}_keys`, `${name}_${searchIdHash}`);
                await this.expire(`${name}_keys`, durationInSeconds);
